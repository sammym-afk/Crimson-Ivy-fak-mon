...
function isWalkable(x, y) {
  ...
  const blockingTile = ["#", "~", "T", "C", "M"].includes(tile);
  const blockingObject = objects.some(object =>
    object.x === x && object.y === y && object.type !== "exit"
  );
  return !blockingTile && !blockingObject;
}
...
function openDetail(title, html, returnTarget = "menu") {
...
function menuAction(type) {
  if (type === "pokedex") {
    ...
    openDetail(
      "Save",
      saved
        ? `<div class="list-card good">Game saved on this device.</div>`
        : `<div class="list-card">The game could not save. Check that Safari is not in Private Browsing.</div>`
    );
  }
}
